...
def create_antiAliasingFilter_main_coefs():
    hi_freq = int(base_freq)
    sample_freq = 48000
    nyq = 0.5 * sample_freq
    if filter_type == 0:
        hi_normal_freq = hi_freq / nyq
        filter_b, filter_a = signal.butter(order, hi_normal_freq, 'low', analog=False)
    else:
    ...
    filter_data = "const double antiAliasingFilter[6] = {"+str(filter_string)+"};\n"
    return filter_data
